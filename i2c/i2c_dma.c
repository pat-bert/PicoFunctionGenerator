...
static int i2c_dma_init_intern(i2c_dma_t *i2c_dma)
{
  ...
  {
    if (!sem_acquire_timeout_ms(&(i2c_dma->semaphore), 0))
    {
    ...
    uint scl_gpio)
{
  ...
  *pi2c_dma = i2c_dma;
  ...
  sem_init(&(i2c_dma->semaphore), 0, 1);

  mutex_init(&(i2c_dma->mutex));
  if (!mutex_is_initialized(&(i2c_dma->mutex)))
  {
    return PICO_ERROR_GENERIC;
  }
    ...
    size_t rbuf_len)
{
  ...
  const bool timeout = !sem_acquire_timeout_ms(&(i2c_dma->semaphore), I2C_TRANSFER_TIMEOUT_MS);
